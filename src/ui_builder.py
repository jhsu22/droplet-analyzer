...
class UIFrame(ctk.CTkFrame):
    ...
    def create_panel(self, parent, title, **frame_kwargs):
        ...
        panel.place(
            x=0,
            y=UIConfig.PADDING_MEDIUM,
            relwidth=1,
            relheight=1,
            height=-UIConfig.PADDING_MEDIUM,
        )
    ...
    def setup_ui(self):
        ...
        button_frame = ctk.CTkFrame(header, fg_color=UIConfig.COLOR_BG_PRIMARY)
        button_frame.pack(
            fill="x",
        ...
        self.video_button.pack(
            side="left", padx=UIConfig.PADDING_SMALL, pady=UIConfig.PADDING_MEDIUM
        )
        ...
        self.start_analysis_button.pack(
            side="right", padx=UIConfig.PADDING_SMALL, pady=UIConfig.PADDING_SMALL
        )
        ...
        # === CONTENT AREA ===
        content_frame = ctk.CTkFrame(self, fg_color=UIConfig.COLOR_BG_PRIMARY)
        content_frame.grid(
            row=1, column=0, sticky="nsew", padx=UIConfig.PADDING_SMALL, pady=0
        )
        content_frame.grid_rowconfigure(
            0, weight=1
        )  # Top row (image, parameters, serial)
        content_frame.grid_rowconfigure(1, weight=1)  # Bottom row (output)
        content_frame.grid_columnconfigure(0, weight=1)

        # === TOP ROW ===
        top_row = ctk.CTkFrame(content_frame, fg_color=UIConfig.COLOR_BG_PRIMARY)
        top_row.grid(
            row=0, column=0, sticky="nsew", padx=UIConfig.PADDING_MEDIUM, pady=0
        )
        ...
        top_row.grid_columnconfigure(
            1, weight=UIConfig.GRID_WEIGHT_PARAMETERS
        )  # Parameter edit area
        top_row.grid_columnconfigure(
            2, weight=UIConfig.GRID_WEIGHT_SERIAL
        )  # Serial connection
        ...
        image_container.grid(
            ...
            column=0,
            sticky="nsew",
            padx=(0, UIConfig.PADDING_MEDIUM),
            pady=(UIConfig.PADDING_SMALL, UIConfig.PADDING_SMALL),
        )
        ...
        video_control_frame.pack(
            side="bottom", fill="x", pady=(UIConfig.PADDING_SMALL, 0)
        )
        ...
        self.video_slider.grid(
            row=0,
            column=0,
            sticky="ew",
            padx=(UIConfig.PADDING_SMALL, UIConfig.PADDING_SMALL),
        ...
        self.frame_number_label.grid(
            row=0, column=1, sticky="e", padx=(0, UIConfig.PADDING_SMALL)
        )
        ...
        parameters_container.grid(
            row=0,
            column=1,
            sticky="nsew",
            padx=(UIConfig.PADDING_MEDIUM, UIConfig.PADDING_MEDIUM),
            pady=(UIConfig.PADDING_SMALL, UIConfig.PADDING_SMALL),
        )
        ...
        self.create_slider(
            param_inner, "sigma", "Gaussian Blur Sigma:", 7, is_float=True
        )
        ...
        self.create_slider(
            param_inner, "clahe_clip_limit", "CLAHE Clip Limit:", 15, is_float=True
        )
        ...
        # Baud rate selection
        ctk.CTkLabel(
            serial_inner, text="Baud Rate:", font=self.master.custom_font
        ).grid(
            row=2,
            column=0,
            sticky="w",
        ...
        self.baud_combo = ctk.CTkComboBox(
            serial_inner, values=SerialConfig.BAUD_RATES, font=self.master.custom_font
        )
        ...
        # === BOTTOM ROW ===
        bottom_row = ctk.CTkFrame(content_frame, fg_color=UIConfig.COLOR_BG_PRIMARY)
        bottom_row.grid(
            row=1, column=0, sticky="nsew", padx=UIConfig.PADDING_MEDIUM, pady=0
        )
        ...
        self.output_text = ctk.CTkTextbox(
            output_panel, fg_color=UIConfig.COLOR_BG_SECONDARY
        )
    ...
    def create_slider(self, parent, name, text, row, is_float=False):
        ...
        value_label = ctk.CTkLabel(
            parent, width=SliderConfig.VALUE_LABEL_WIDTH, font=self.master.custom_font
        )
        value_label.grid(
            row=row,
            column=2,
        ...
        if name == "filter_size":
            slider.configure(
                command=lambda v, n=name: self.update_parameter(n, v, is_float=True)
            )
